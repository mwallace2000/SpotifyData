...
def main():
	...
	parser.add_argument('--dump', default='episodes', choices=['liked,playlists', 'playlists,liked', 'playlists', 'liked', 'episodes'],
	...
	if 'episodes' in args.dump:
		logging.info('Loading podcast episodes...')
		liked_tracks = spotify.list('me/episodes'.format(user_id=me['id']))
		playlists += liked_tracks

	logging.info('Loading shows...')
	liked_tracks = spotify.list('me/shows'.format(user_id=me['id']))
	shows = []
	shows += liked_tracks
	shows = [[i["show"]["name"], i["show"]["total_episodes"]] for i in shows]
	shows_df = pd.DataFrame(shows, columns = ["Name", "Total Episodes"])

	try:
		a = pd.read_excel("podcasts.xlsx").fillna(0)
		a = a.drop(columns = [i for i in a.columns if ("Episodes" in i or "Minutes" in i) and i != "Total Episodes"])
		print(a)
	...
	for i in playlists:
		episodes.append(i)
		names.append(i["episode"]["show"]["name"])
		times.append(i["episode"]["duration_ms"]/1000/60)
	names_df = pd.DataFrame(names, columns = ["Name"])
	names_df["Episodes"] = 1
	names_df["Total Time (Minutes)"] = times
	try:
		a = a.rename(columns = {"Total Episodes":"Total Episodes (Old)"})
	except Exception as e:
		print(e)	

	grouped = names_df.groupby("Name").sum().reset_index()
	grouped["Total Time (Minutes)"] = grouped["Total Time (Minutes)"].round()

	try:
		grouped = grouped.merge(a, on = "Name", how = "outer")
	except Exception as e:
		print(e)
	grouped = grouped.merge(shows_df, on = "Name", how = "inner")
	filtered = grouped[(grouped["Episodes"] < 50) & (grouped["Total Episodes"] != grouped["Total Episodes (Old)"])]
	print(filtered)
	grouped = grouped.sort_values(["Episodes", 'Total Time (Minutes)', "Name"], ascending = False)
	grouped = grouped.fillna(0)

	writer = pd.ExcelWriter(args.file) 
	grouped.to_excel(writer, sheet_name='Saved Episode Counter', index=False, na_rep='NaN')

	# Auto-adjust columns' width
	for column in grouped:
		column_width = max(grouped[column].astype(str).map(len).max(), len(column))
		col_idx = grouped.columns.get_loc(column)
		writer.sheets['Saved Episode Counter'].set_column(col_idx, col_idx, column_width)

	writer.save()
	with open("episodes.txt", "w") as my_file:
		for i in episodes:
			my_file.write(str(i)+"\n")
