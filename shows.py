...
def main():
	...
	logging.info('Loading followed shows...')
	liked_tracks = spotify.list('me/shows'.format(user_id=me['id']))
	playlists += liked_tracks

	uris = [(i["show"]["uri"], i["show"]["name"]) for i in playlists]
	uris = pd.DataFrame(uris, columns = ["URI", "Title"])
	print(uris)
	uris.to_csv("uris.csv")
	print(playlists[0])
	...
	episode_df["added_at"] = [episode["added_at"] for episode in playlists]
	episode_df["uri"] = [episode["episode"]["show"]["uri"] for episode in playlists]
	print(episode_df)
	loaded_uris = pd.read_csv("uris.csv")
	episodes = spotify.list('shows/{id}/episodes'.format(id=loaded_uris.iloc[0]["URI"][13:]))
